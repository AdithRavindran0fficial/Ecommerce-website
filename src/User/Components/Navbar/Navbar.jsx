import React, { useContext, useEffect, useState } from 'react'
import { Link, Outlet, useNavigate } from 'react-router-dom'
import {headerLogo} from "../../../assets/assets/images"
import {hamburger} from "../../../assets/assets/icons"
import { Cart_Context } from '../../Context/CartContext'
import axios from 'axios'
import { UserRound } from 'lucide-react';
import Profile from '../Profile/profile'
import { searchContext } from '../../Context/Searchprovider'


function Navbar() {
    let navigate = useNavigate()
    let {setterm,term} = useContext(searchContext)
    let {size,userLog,setloguser,id} = useContext(Cart_Context)
    let [name,setname] = useState([])
    let [ismenuopen,setmenu]  = useState(false)
    let [showProfile,setProfile] = useState(false)
    let [email,setemail] = useState([])
    useEffect(()=>{
        axios.get(`http://localhost:5000/Users/${id}`)
        .then((res)=>{setname(res.data.name)
        setemail(res.data.email)
    })
        
    },[])
    const toggleMenu = ()=>{
        setmenu(!ismenuopen)
    }
    if(id){
        setloguser(true)
        

    }
    const handlelog=()=>{
        setloguser(false)
        localStorage.removeItem("id")
        setProfile(false)
        navigate("/")
        alert("logged out")
        

        
    }
    const handle_profile =()=>{
        setProfile(true)
    }
    const handle_input = (e)=>{
    setterm(e.target.value.toLowerCase())
    navigate("/collections")
   
    }
  return (
    <header className=' border-black px-12 py-8  absolute w-full  h-9 '>
        <nav className=' flex  justify-between items-center -mt-4  max-container '>
            <Link to="/">
            <img src={"data:image/png;base64,"}
             alt='Logo' width={130}height={29}></img>
            </Link>
        <div className="w-full sm:w-auto mt-8 mb-8">
            <input
                 type="text"
                 value={term}
                 onChange={handle_input}
                 className="w-full sm:w-56 border rounded-lg shadow-sm focus:outline-none focus:border-blue-500 px-4 py-2"
                 placeholder="Search for shoes"/>
      </div>
            
            <ul className=' flex justify-center items-center gap-16 max-lg:hidden'>
                <li><Link to={"/"} ><p className='font-light text-lg text-slate-gray'>Home</p></Link></li>
                <li><Link to={"/men"}><p className='font-light text-lg text-slate-gray'>Men</p></Link></li>
                <li><Link to={"/women"}><p className='font-light text-lg text-slate-gray'>Women</p></Link></li>
                <li><Link to={"/collections"}><p className='font-light text-lg text-slate-gray'>Collection</p></Link></li>
            </ul>
            <ul className=' flex gap-4 justify-end pl-32 list-none max-lg:hidden'>
                <li className='ml-3'><Link to={"/cart"}><p className='font-bold text-lg text-slate-gray'><i className="fas fa-shopping-cart"></i><span className='text-white rounded-tl-full border bg-red-600'>{size}</span></p></Link></li>
                {userLog?<li><div className="w-15 h-15 rounded-full overflow-hidden  border-gray-300" onClick={handle_profile}>
                <UserRound/>
       
      </div>
    </li>:
                <li><Link to={"/login"}><p className='font-bold text-lg text-slate-gray'>Login</p></Link></li>}
                {showProfile&& <Profile name={name} email={email} handlelog={handlelog} onclose = {()=>setProfile(false)}/> }
                </ul> 
            <div className='hidden max-lg:block'>
                <button onClick={toggleMenu}>
                <img src={hamburger} width={25} height={25}></img>
                </button>
            </div>
        </nav>
        {ismenuopen &&(
            <div className=" bg-white absolute top-16 left-0 w-full shadow-md">
                <ul className='flex flex-col items-center'>
                <li className='py-2'><Link to={"/"} onClick={toggleMenu}><p className='font-light text-lg text-slate-gray'>Home</p></Link></li>
                        <li className='py-2'><Link to={"/men"} onClick={toggleMenu}><p className='font-light text-lg text-slate-gray'>Men</p></Link></li>
                        <li className='py-2'><Link to={"/women"} onClick={toggleMenu}><p className='font-light text-lg text-slate-gray'>Women</p></Link></li>
                        <li className='py-2'><Link to={"/collections"} onClick={toggleMenu}><p className='font-light text-lg text-slate-gray'>Collection</p></Link></li>
                        <li className='py-2'><Link to={"/cart"} onClick={toggleMenu}><p className='font-bold text-lg text-slate-gray'><i className="fas fa-shopping-cart"></i>{size > 0 && <span className='ml-2 bg-red-600 text-white rounded-full px-2 text-xs'>{size}</span>}</p></Link></li>
                       {userLog?<li><Link to={"/login"}><div className="w-15 h-15 rounded-full overflow-hidden border-2 border-gray-300"onClick={handlelog}>
                       <UserRound/>
                       
      </div>
    </Link></li> :<li className='py-2'><Link to={"/login"} onClick={toggleMenu}><p className='font-bold text-lg text-slate-gray'>Login</p></Link></li>}
                </ul>
            </div>
        )}
        <Outlet/>
    </header>


    
  )
}

export default Navbar